...
class SCMPrior(Prior):
    ...
    def get_batch(self, batch_size: Optional[int] = None) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        for gp_idx in range(num_gps):
            ...
            for subgp_idx in range(num_subgps_in_gp):
                ...
                subgp_prior_type, subgp_tree_model = self.get_prior()
                ...
                for ds_idx in range(actual_subgp_size):
                    ...
                    # If we picked a tree_scm variant, set its tree_model
                    if subgp_prior_type == "tree_scm" and subgp_tree_model is not None:
                        params["tree_model"] = subgp_tree_model
    ...
    def get_prior(self) -> Tuple[str, Optional[str]]:
        """
        Determine which prior type (and tree model, if applicable) to use.

        For 'mix_scm', we sample:
          - 'mlp_scm'             with prob 0.5
          - 'tree_scm' + ExtraTrees   with prob 0.25
          - 'tree_scm' + XGBoost      with prob 0.15
          - 'tree_scm' + DecisionTree with prob 0.10

        Returns
        -------
        (prior_type, tree_model)
            prior_type: 'mlp_scm' or 'tree_scm'
            tree_model: one of {'extra_trees', 'xgboost', 'decision_tree'} or None for mlp_scm
        """
        if self.prior_type == "mitra_mix_scm":
            probs = self.fixed_hp.get("mix_probas", [0.5, 0.2, 0.15, 0.10, 0.05])
            choices = ["mlp_scm", "tree_extra_trees", "tree_xgboost", "tree_decision_tree", "tree_random_forest"]
            choice = np.random.choice(choices, p=probs)

            if choice == "mlp_scm":
                return "mlp_scm", None
            elif choice == "tree_extra_trees":
                return "tree_scm", "extra_trees"
            elif choice == "tree_xgboost":
                return "tree_scm", "xgboost"
            elif choice == "tree_decision_tree":
                return "tree_scm", "decision_tree"
            elif choice == "tree_random_forest":
                return "tree_scm", "random_forest"
            else:
                return "mlp_scm", None
        if self.prior_type == "mix_scm":
            return np.random.choice(["mlp_scm", "tree_scm"], p=self.fixed_hp.get("mix_probas", [0.7, 0.3])), None
        prior_type = self.prior_type
        tree_model = self.fixed_hp.get("tree_model", "xgboost") if prior_type == "tree_scm" else None
        return prior_type, tree_model
