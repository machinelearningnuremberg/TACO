...
from taco.model.tabpfn_arch.base import (
...
from taco.model.tabpfn_arch.constants import (
    PROBABILITY_EPSILON_ROUND_ZERO,
    SKLEARN_16_DECIMAL_PRECISION,
    XType,
    YType,
)
from taco.model.tabpfn_arch.inference import InferenceEngine, InferenceEngineBatchedNoPreprocessing
from taco.model.tabpfn_arch.model.loading import (
    load_fitted_tabpfn_model,
    save_fitted_tabpfn_model,
)
from taco.model.tabpfn_arch.preprocessing import (
...
from taco.model.tabpfn_arch.utils import (
    _fix_dtypes,
    _get_embeddings,
...
class TabPFNClassifier(ClassifierMixin, BaseEstimator):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        fit_mode: Literal[
            "low_memory",
            "fit_preprocessors",
            "fit_preprocessors_with_compression",
            "fit_with_cache",
            "batched",
        ] = "fit_preprocessors",
        ...
        compression_method: Literal["knn", "random"] | None = None,
        compression_rate_percentage: float = 10.0,
        **kwargs: Any,
    ) -> None:
        ...
        Args:
            ...
            fit_mode:
                ...
                - If `"fit_preprocessors_with_compression"`, similar to `"fit_preprocessors"`,
                  but during prediction the training data is compressed (via random sampling
                  or K nearest neighbors) to reduce context size. Compression is only applied
                  when the model does not already have a compressor. Ideal for large training
                  datasets where you want to reduce memory usage during inference.
                - If `"fit_with_cache"`, the data is preprocessed and cached once during
                  ...
                  with the same training data. When using a compressor model
                  (TabPFNClassifierWithCompressor), this automatically uses the
                  compressor cache engine which caches both the compressed context and
                  predictor KV cache.
                - If `"fit_with_compressor_cache"`, explicitly use the compressor cache
                  engine. This caches the compressor output (compressed context) and
                  the predictor KV cache, allowing for very fast repeated inference on
                  the same training data with different test sets. Requires a compressor
                  model.
        ...
        if self.device == "cuda" and not torch.cuda.is_available():
            logging.warning(
                "CUDA is not available. Falling back to CPU. Set device='auto' or device='cpu' to avoid this warning."
            )
            self.device = "cpu"
        ...
        self.fit_mode: Literal["low_memory", "fit_preprocessors", "fit_preprocessors_with_compression", "fit_with_cache", "fit_with_compressor_cache"] = (
            fit_mode
        )
        ...
        self.compression_rate_percentage = compression_rate_percentage
    ...
    def fit(self, X: XType, y: YType) -> Self:
        ...
        logging.getLogger(__name__).info(
            "Creating inference engine with fit_mode=%r, shuffle_rows=%s, training data shape=%s",
            self.fit_mode,
            self.shuffle_rows,
            X.shape,
        )
    ...
    def forward(  # noqa: C901, PLR0912
        ...
        *,
        ...
        if self.fit_mode in ["fit_preprocessors", "fit_preprocessors_with_compression", "batched"]:
