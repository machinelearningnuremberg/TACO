...
from taco.model.tabpfn_arch.inference import (
    InferenceEngine,
    ...
    InferenceEngineWithCompression, InferenceEngineChunkedCompressor,
)
from taco.model.tabpfn_arch.model.bar_distribution import FullSupportBarDistribution
from taco.model.tabpfn_arch.model.loading import load_model_criterion_config
...
from taco.model.tabpfn_arch.settings import settings
...
def create_inference_engine(  # noqa: PLR0913
    *,
    ...
    cat_ix: list[int],
    fit_mode: Literal["low_memory", "fit_preprocessors", "fit_preprocessors_with_compression", "fit_with_cache", "fit_with_compressor_cache", "batched", "fit_with_chunking"],
    device_: torch.device,
    ...
    if hasattr(model, "core") and hasattr(model.core, "clear_eval_cache"):
        model.core.clear_eval_cache()

    
    if fit_mode == "fit_with_compressor_cache":
        # Use compressor cache engine for compressor-based models
        engine = InferenceEngineCacheCompressorKV.prepare(
            X_train=X_train,
            y_train=y_train,
            cat_ix=cat_ix,
            model=model,
    ...
    elif fit_mode == "fit_preprocessors_with_compression":
        # Check if model has compressor - if so, fall back to standard preprocessing
        has_compressor = (
            hasattr(model, "core")
        ...
        if has_compressor:
            ...
            engine = InferenceEngineWithCompression.prepare(
                X_train=X_train,
                y_train=y_train,
                cat_ix=cat_ix,
                ensemble_configs=ensemble_configs,
                ...
                save_peak_mem=memory_saving_mode,
                inference_mode=inference_mode,
                compression_method=compression_method,  # Default to KNN, can be made configurable
                compression_rate_percentage=compression_rate_percentage,
                shuffle_rows=shuffle_rows,
            )
    elif fit_mode == "fit_with_cache":
        # Check if model has compressor - if so, use compressor cache engine
        has_compressor = (
            hasattr(model, "core")
            and hasattr(model.core, "use_compressor")
            and model.core.use_compressor
        ...
        if has_compressor:
            logger.info("Using compressor cache inference engine.")
            engine = InferenceEngineCacheCompressorKV.prepare(
                X_train=X_train,
                y_train=y_train,
                cat_ix=cat_ix,
            ...
            logger.info("Using standard cache inference engine.")
    ...
    elif fit_mode == "fit_with_chunking":
        engine = InferenceEngineChunkedCompressor.prepare(
            X_train=X_train,
            y_train=y_train,
            cat_ix=cat_ix,
            model=model,
            ...
            shuffle_rows=shuffle_rows,
            # knobs:
            train_chunk_size=None,  # or set e.g. 10_000
            max_test_batch=None,  # or set e.g. 2048/4096
            stats_sample_size_override=None,  # defaults to model.stats_sample_size, usually 3000
        )
    else:
