...
warnings.filterwarnings(
    "ignore",
    message=r"Fields in config not in Config class:.*",
...
DEFAULT_TACO_CONFIG_FILENAME = "taco_tabpfn_config.json"
...
class ModelSource:
    ...
    def get_classifier_v2(cls) -> ModelSource:
        ...
        return cls(
            ...
            default_filename="tabpfn-v2-classifier-v2_default.ckpt",
            filenames=filenames,
        )
...
def coerce_model_config(config: ModelConfig | dict) -> ModelConfig:
    """Normalize stored model config objects/dicts to a ModelConfig instance."""
    if isinstance(config, ModelConfig):
        return config
    if isinstance(config, dict):
        return ModelConfig.from_dict(ModelConfig.upgrade_config(config))
    raise TypeError(f"Expected ModelConfig or dict, got {type(config).__name__}")


def load_default_taco_model_config() -> ModelConfig:
    """Load the packaged TabPFN architecture config used by TACO/POT checkpoints."""
    config_text = files(__package__).joinpath(DEFAULT_TACO_CONFIG_FILENAME).read_text(encoding="utf-8")
    return coerce_model_config(json.loads(config_text))


def load_model_criterion_from_config(
    ...
    *,
    ...
    """Build a TabPFN architecture directly from config."""
    model_config = coerce_model_config(config)
    ...
    model.cache_trainset_representation = cache_trainset_representation
    return model, criterion, model_config
...
def load_model(
    *,
    ...
    if overwrite_config and new_config is not None:
        ...
        for key, value in new_config.items():
            if hasattr(config, key):
                ...
                raise ValueError(f"Config has no attribute '{key}' to overwrite.")   
